...
export function GanttView({ projects, tasks, onProjectClick }: GanttViewProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            onClick={() => {
              ...
              // Calculer la position d'aujourd'hui dans le mois
              const viewStart = startOfMonth(today)
              const viewEnd = endOfMonth(today)
              const totalDays = eachDayOfInterval({ start: viewStart, end: viewEnd }).length
              const dayOffset = differenceInDays(today, viewStart)
              const todayPercent = dayOffset / totalDays
              
              // Seulement changer de mois si nécessaire
              if (currentMonth !== todayMonth || currentYear !== todayYear) {
                setCurrentDate(today)
              }
              
              // Fonction pour défiler vers aujourd'hui
              const scrollToToday = () => {
                const scrollContainer = ganttContainerRef.current?.querySelector('.overflow-x-auto') as HTMLElement
                if (scrollContainer) {
                  const clientWidth = scrollContainer.clientWidth
                  const scrollWidth = scrollContainer.scrollWidth
                  const projectColumnWidth = 320 // w-80 = 320px
                  
                  // Position d'aujourd'hui en pixels
                  const ganttWidth = scrollWidth - projectColumnWidth
                  const todayPixelPosition = projectColumnWidth + (todayPercent * ganttWidth)
                  
                  // Centrer sur aujourd'hui
                  const scrollTo = todayPixelPosition - (clientWidth / 2)
                  
                  // Scroll direct pour plus de stabilité
                  scrollContainer.scrollLeft = Math.max(0, scrollTo)
                }
              }
              
              // Si on change de mois, attendre le rendu, sinon défiler directement
              if (currentMonth !== todayMonth || currentYear !== todayYear) {
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[1400px] relative">
            ...
            <div className="flex border-b border-blue-400/20 bg-gradient-to-r from-[#2d4a6f] to-[#1e3a5f]">
              ...
              <div className="w-80 min-w-80 p-4 border-r border-blue-400/20">
                <span className="text-base font-bold text-amber-400 uppercase tracking-wide">
                  Projets & Tâches
                </span>
              </div>
                ...
                {viewMode === 'month' ? (
                  ...
                  dateRange.days.map((day, index) => (
                    <div
                      ...
                      className={`flex-1 min-w-10 p-2 text-center border-r border-blue-400/10 ${
                    ...
                    >
                      ...
                      <div className={`text-base font-bold ${isToday(day) ? 'text-amber-400' : 'text-white'}`}>
                        {format(day, 'd')}
                      </div>
                    </div>
                  ))
                  ...
                  dateRange.weeks.map((week, index) => (
                    <div
                      key={index}
                      className="flex-1 p-3 text-center border-r border-blue-400/10"
                    >
                      <div className="text-sm text-blue-300 font-medium">Sem {index + 1}</div>
            ...
            <div className="divide-y divide-blue-400/10 relative">
              ...
              {todayLineLeft !== null && (
                ...
                >
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-amber-500 text-white text-sm font-bold px-4 py-1.5 rounded-b-lg whitespace-nowrap shadow-lg">
                  ...
                  return (
                    <div key={project.id} className="hover:bg-blue-500/5">
                      ...
                      <div className="flex items-stretch">
                        <div 
                          className="w-80 min-w-80 p-4 border-r border-blue-400/20 cursor-pointer"
                        ...
                        <div className="flex-1 relative h-16">
                          {(() => {
                            ...
                            return (
                              <div
                                className={`absolute top-2 h-12 rounded-lg ${getStatusColor(project.status)} flex items-center justify-between px-4 shadow-lg cursor-pointer group`}
                                style={barStyle}
                                onClick={() => onProjectClick?.(project.id)}
                              ...
                              >
                                <span className={`text-base truncate ${getBarTextStyle(project.status)}`}>
                                  {project.name}
                                </span>
                                {/* Dates affichées dans la barre - très visibles */}
                                <span className={`text-sm ml-3 whitespace-nowrap bg-black/30 px-3 py-1 rounded ${getBarTextStyle(project.status)}`}>
                      ...
                      {projectTasks.map(task => {
                        ...
                        return (
                          <div key={task.id} className="flex items-stretch bg-blue-900/10">
                            <div className="w-80 min-w-80 p-3 pl-10 border-r border-blue-400/20">
                              <div className="flex items-center gap-2">
                                <div className={`w-3 h-3 rounded-full ${getStatusColor(task.status)}`} />
                                <span className="text-base font-medium text-blue-100 truncate">
                                  {task.title}
                                </span>
                              </div>
                              {task.dueDate && (
                                <div className="text-sm text-blue-300 ml-5 mt-1 font-medium">
                            ...
                            <div className="flex-1 relative h-12">
                              {(() => {
                                ...
                                if (!taskBarStyle) {
                                  ...
                                  return (
                                    <div className="absolute top-2 h-8 left-0 right-0 flex items-center justify-center">
                                ...
                                return (
                                  <div
                                    className={`absolute top-2 h-8 rounded ${getStatusColor(task.status)} flex items-center justify-between px-3`}
                                    style={taskBarStyle}
                                  ...
                                  >
                                    <span className={`text-sm truncate ${getBarTextStyle(task.status)}`}>
                                      {task.title}
                                    </span>
                                    {/* Dates sur la barre tâche - très visibles */}
                                    <span className={`text-xs ml-2 bg-black/30 px-2 py-0.5 rounded whitespace-nowrap ${getBarTextStyle(task.status)}`}>
      ...
      {/* Légende */}
      <div className="bg-gradient-to-br from-[#1e3a5f] to-[#1a2744] rounded-xl border border-blue-400/30 p-4 shadow-lg shadow-blue-500/10">
        <h3 className="text-base font-bold text-amber-400 uppercase tracking-wide mb-3">
          Légende
        </h3>
        <div className="flex flex-wrap gap-6">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded bg-blue-500" />
            <span className="text-base text-blue-200 font-medium">En cours</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded bg-red-500" />
            <span className="text-base text-blue-200 font-medium">En retard</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded bg-green-500" />
            <span className="text-base text-blue-200 font-medium">Validé / Terminé</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded bg-gray-400" />
            <span className="text-base text-blue-200 font-medium">À faire</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-5 h-0.5 bg-amber-500" />
            <span className="text-base text-blue-200 font-medium">Aujourd&apos;hui</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-gradient-to-br from-[#1e3a5f] to-[#2d4a6f] rounded-xl p-4 border border-blue-400/30 shadow-lg shadow-blue-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-blue-200 font-medium">Projets affichés</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#1e3a5f] to-[#2d4a6f] rounded-xl p-4 border border-blue-400/30 shadow-lg shadow-blue-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-blue-200 font-medium">Tâches totales</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#1e3a5f] to-[#2d4a6f] rounded-xl p-4 border border-blue-400/30 shadow-lg shadow-blue-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-blue-200 font-medium">Tâches en retard</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#1e3a5f] to-[#2d4a6f] rounded-xl p-4 border border-blue-400/30 shadow-lg shadow-blue-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-blue-200 font-medium">Tâches validées</p>
            </div>
          </div>
        </div>
      </div>
